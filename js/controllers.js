...
controller('MyCtrl1', [function() {
    ...
    .controller('AboutCtrl', ['$rootScope', function($rootScope) {
        $rootScope.openPage = function() {
            var url = "http://www.kryptosmobile.com";
            ...
            var iabRef = window.open(url, "_blank", "location=yes,hidden=no;enableViewportScale=yes");
                ...
                window.plugin.email.isServiceAvailable(
                    function(isAvailable) {
                        ...
                        if (isAvailable) {
                            ...
                            navigator.notification.alert("Please check your Email configurations", null, "Report Issue Email", "OK");
                        }
                    }
                );
                ...
                window.plugin.email.isServiceAvailable(
                    function(isAvailable) {
                        if (isAvailable) {
                            ...
                            navigator.notification.alert("Please check your Email configurations", null, "Feedback Email", "OK");
                        }
                    }
                );
            } else {
    ...
    .controller('DeviceCtrl', ['$scope', '$rootScope', function($scope, $rootScope) {
        if (window.device) {
            ...
            $scope.devicemodel = window.device.model.replace(/,/g, '.');
        } else {
            ...
            $rootScope.login = function() {
                ...
                window['authFunction'](username, password, $http, $rootScope, function(data) {
                    if (data.error) {
                        ...
                        if ($rootScope.loggedin == true) {} else {
    ...
    .controller('HomeCtrl', ['$rootScope', '$scope', '$http', '$location', '$window', '$sce', '$route', '$compile',
        function($rootScope, $scope, $http, $location, $window, $sce, $route, $compile) {
            $rootScope.sysdate = new Date().getTime();
            ...
            if ($rootScope.loggedin) {
                $rootScope.loginclass = "fa fa-lock fa-2x";
                $rootScope.logintext = "Logout";
            } else {
                $rootScope.loginclass = "fa fa-unlock fa-2x";
    ...
    .controller('AppCtrl', ['$scope', '$routeParams', '$compile', '$http', '$rootScope', '$sce', '$window',
        ...
        function($scope, $routeParams, $compile, $http, $rootScope, $sce, $window, $location) {
            ...
            try {
                ...
                $.unblockUI();
                apprise("Unknown error occurred while processing the request.!", {
                    'verify': false,
                    'textYes': "Ok"
                }, function(r) {
                    $rootScope.back();
            ...
            $scope.loadApps = function(token) {
                ...
                success(function(dt) {
                    ...
                    if (status == 403) {
                        ...
                        $http.post(url + "/api/authenticate", {
                            username: username,
                            password: password
                        }).
                        success(function(data) {
    ...
    .controller('AppUpdatedCtrl', ['$rootScope', '$scope', '$http', '$location', '$window', '$sce', '$route', '$compile', '$routeParams',
        function($rootScope, $scope, $http, $location, $window, $sce, $route, $compile, $routeParams) {
            try {
                if (!$rootScope.loggedIn) {
                    if ($.jStorage.get('tenantInfo').guestApp) {
                        ...
                        window.location.href = '#/app/Menu82/Menu82'; /* Redirect user to home page */
                    } else {
                        window.location.href = '#/app/SignIn279/SignIn279'; /* Redirect user to signin page since it doesn't have guest applets */
                    }
                } else {
